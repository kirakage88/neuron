...
def load_model(model: str, path):
    match model:
        case 'linear':
            loaded_model = model_class.LinearRegressionModel()
        case 'binary':
            loaded_model = model_class.CircleModel()
        case 'multi':
            loaded_model = model_class.BlobModel()
        case 'mnist':
            loaded_model = model_class.MNIST()
        case 'CVD':
            weights = torchvision.models.EfficientNet_B0_Weights.DEFAULT
            loaded_model = models.efficientnet_b0(weights=weights)
            ...
            loaded_model.classifier = torch.nn.Sequential(
                ...
                torch.nn.Linear(in_features=1280,
                                out_features=1,
                                # same number of output units as our number of classes (for binary set to 1)
                                bias=True))
        case 'waste':
            weights = torchvision.models.EfficientNet_B4_Weights.DEFAULT
            loaded_model = models.efficientnet_b4(weights=weights)
            for param in loaded_model.features.parameters():
                param.requires_grad = False  # Freeze model first then adjust the output_features
...
def predict_image_efficient_net(model: torch.nn.Module,
    ...
    plt.figure()
    plt.imshow(img)
    plt.title(f"Pred: {class_names[target_image_pred_label]} | Prob: {target_image_pred_probs.item():.2f}%")
    plt.axis(False)
    plt.show()
...
def predict_image_mnist(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        ...
        plt.figure()
        plt.imshow(img)
        plt.title(f"Pred: {class_names[target_image_label]} | Prob: {target_image_pred_probs.max():.2f}%")
        plt.axis(False)
        plt.show()


def predict_image_waste(model: torch.nn.Module,
                                image_path: str,
                                class_names: List[str] = ['biodegradable', 'non-biodegradable', 'recyclable'],
...
def channel_reader(img_path: str):
